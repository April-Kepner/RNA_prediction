...
import vocabulary
from network_res import Network
...
def transfer(batches):
    ...
    for i in range(len(batches)):
        seq, dot, y = batches[i]
        new_seq = np.pad(seq,(0, max_length - seq.size),'constant', constant_values=(0))
        new_dot = np.pad(dot,(0, max_length - seq.size),'constant', constant_values=(0))
        new_y = np.pad(y,(0, max_length - seq.size),'constant', constant_values=(0))
        new_batches.append((new_seq, new_dot, new_y))
        tmp = random.randint(0,1)
        if (tmp == 0):
            new_batches.append((new_seq, new_dot, new_y))
        else:
            new_batches.append((new_seq[::-1], new_dot[::-1], new_y[::-1]))
...
def run_test(args):
    ...
    for data in test_reader():
        ...
        f = open('predict.files/'+str(count)+'.predict.txt','w')
        count += 1
...
def main():
    ...
    subparser.add_argument("--batch-size", default=32)
    ...
    subparser.add_argument("--dropout", type=float, default=0)
    ...
    subparser.add_argument("--logfile", default="test_log.txt")
    subparser.add_argument("--batch-size", default=1)
    subparser.add_argument("--dmodel", type=int, default=64)
    subparser.add_argument("--layers", type=int, default=8)
