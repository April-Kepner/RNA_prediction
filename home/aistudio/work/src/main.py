...
import vocabulary
from network_attention import Network
            ...
            if current_processed >= check_every:
                ...
                for data in val_reader():
                    ...
                    loss, pred = exe.run(test_program,
                                         feed=feeder.feed(transfer(data)),
                                         fetch_list=[avg_loss.name, predictions.name],
                                         return_numpy=False
                    )
                    loss = np.array(loss)
...
def main():
    ...
    subparser.add_argument("--batch-size", default=4)
    ...
    subparser.add_argument("--layers", type=int, default=4)
